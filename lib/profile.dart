...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Duration calculateTimeDifference(Timestamp? registeredDate) {
    if (registeredDate != null) {
      ...
      DateTime registeredDateOnly = DateTime(registeredDateTime.year, registeredDateTime.month, registeredDateTime.day);
  ...
  Future<void> readUserData(String uid) async {
    try {
      ...
      Map<String, dynamic>? userData = matchingDoc.data() as Map<String, dynamic>?;
  ...
  Widget build(BuildContext context) {
    ...
    final UserModel routeUser = ModalRoute.of(context)!.settings.arguments as UserModel;
    ...
    if (registeredDate != null) {
      ...
      formattedBeginDate = registeredDate != null ? DateFormat('yyyy.MM.dd').format(registeredDate!.toDate()) : '';
    }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: <Widget>[
          ...
          Text(
            user.name, textAlign: TextAlign.center, style: TextStyle(fontSize:28, fontWeight: FontWeight.bold),
          ),
          const SizedBox(height: 20.0),
          Text(
            user.email, textAlign: TextAlign.center, style: TextStyle(fontSize: 16),
          ),
          ...
          Align(
            ...
            child: Padding(
              padding: const EdgeInsets.only(right: 15.0),
              child: Text(
                formattedTimeDifference != null ? "D+" + formattedTimeDifference! : '',
                textAlign: TextAlign.center,
                style: TextStyle(fontSize: 16),
              ),
            ),
